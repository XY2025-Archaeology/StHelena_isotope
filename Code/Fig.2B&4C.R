...
######################################This script is for creating Figure 2B and Figure 4C#####################################################################################
...
####################################Modeling the geographic origins of individuals in Groups 1–6 shown in Figure 2B and of one individual with two teeth shown in Figure 4C####################
...
r_eck<-terra::rast("results/wc2.1_30s_elev.tif") #Any global raster file in the WGS84 projection can be used
...
df<-read_excel("data/Fig2B_and_4C_data.xlsx")
df_1<-df[c(1:8),]
...
df_2<-read_excel("data/Fig2B_and_4C_data.xlsx")
df_3<-df[c(9:12),]
...
#######################################Plotting one individual with paried teeth shown in Figure 4C###########################################
# Shared settings
base_theme <- theme_bw() +
  theme(
...
example_13 <- ggplot() +
  geom_tile(
    data = iso_examples_plot,
    aes(x = x, y = y, fill = cut(I_242_1, breaks01))
	) +
  geom_sf(data = afr, fill = NA, color = "grey90", linewidth = 0.25) +
  geom_sf(data = river1, color = "#00FFFF", linewidth = 0.2) +
  annotation_scale(
    location = "bl",
    width_hint = 0.2,
    text_cex = 0.4) +
...
example_14 <- ggplot() +
  geom_tile(
    data = iso_examples_plot,
    aes(x = x, y = y, fill = cut(I_242_2, breaks01))
  ) +
  geom_sf(data = afr, fill = NA, color = "grey90", linewidth = 0.25) +
  geom_sf(data = river1, color = "#00FFFF", linewidth = 0.2) +
  annotation_scale(
...
# Arrange plots
Figure4C<-ggarrange(example_13,example_14,nrow=1,ncol=2,common.legend = T,legend = "right")

# Export the figure as a high-resolution PDF
ggsave("figures/Fig.4C.pdf",Figure4C,width=9.5, height=6,units="cm")
